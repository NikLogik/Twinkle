...
class InitialPreprocessingDataLoader {
    ...
    private void loadFire() {
        ...
        double convWindDirection = GeodeticCalculator.convertDirection(preprocessingData.getWeather().getWindDirection());
        ...
        Map<Id<Agent>, Agent> idAgentMap = factory.generateFireFront(fire.getCenterPoint(), fire.getAgentDistance(),
                                            fire.getPerimeter(), convWindDirection);
        ...
        for (Agent agent : fire.getTwinkles().values()){
            calculator.calculateSpeedOfSpreadWithArbitraryDirection(speed, agent, convWindDirection);
        }
    }
}
